...
namespace Orleans.SwaggerGen
{
    public class SwaggerGenOptions
    {
        ...
        public void Assemblys(string name, Assembly assembly)
        {
            _swaggerGeneratorSettings.ApiAssemblys.Add(name,assembly);
        } 
        /// <summary>
        ...
        internal ISwaggerProvider CreateSwaggerProvider()
        {
            ...
            foreach (var xmlDocFactory in _xmlDocFactories)
            {
                ...
                _schemaRegistrySettings.SchemaFilters.Insert(0, new XmlCommentsSchemaFilter(xmlDoc));
                _swaggerGeneratorSettings.OperationFilters.Insert(0, new XmlCommentsOperationFilter(xmlDoc));

                if (_includeControllerXmlComments)
                    _swaggerGeneratorSettings.DocumentFilters.Insert(0, new XmlCommentsDocumentFilter(xmlDoc));
            }

            var jsonSerializerSettings = new JsonSerializerSettings();
            var schemaRegistryFactory = new SchemaRegistryFactory(jsonSerializerSettings,
                _schemaRegistrySettings
            );

            return new SwaggerGenerator(
                schemaRegistryFactory,
                new ApiDescriptionCollectionProvider(_swaggerGeneratorSettings),
                _swaggerGeneratorSettings
            );
        }
